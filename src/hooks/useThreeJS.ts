     ...
     const createDebugInfoOverlay = (params: HandrailParameters, manualData: Record<number, number>, calculatedData: Record<number, number>) => {
       ...
       const debugInfo = [
         ...
         `Top Easement: ${topEasementStart.toFixed(1)}° to 220°`,
          ...
          `Inner Line End: ${(insidePitchBlockOffset + safeTotalRise).toFixed(3)}"`,
          `Inner Line Rise: ${safeTotalRise.toFixed(3)}" over 10.5" arc`,
          `Inner Pitch Block: ${insidePitchBlockOffset.toFixed(3)}"`,
          `Height Match: ${Math.abs((insidePitchBlockOffset + safeTotalRise) - targetFinalHeight) < 0.001 ? '✓ PERFECT' : '✗ MISMATCH'}`,
     ...
     if (!manualRiseData || !calculatedRiseData || typeof manualRiseData !== 'object' || typeof calculatedRiseData !== 'object') {
      ...
      for (let i = 0; i <= steps; i++) {
        ...
        if (segmentPosition <= parameters.bottomLength) {
          // Bottom easement: simplified smooth transition without split-half complexity
          ...
          // Top easement: simplified smooth transition without split-half complexity
          if (parameters.topLength <= 0) {
            ...
            // Simple, smooth easement from spiral to final position
       ...
       if (segmentPosition <= parameters.bottomLength) {
         ...
         if (parameters.bottomLength <= 0) {
           ...
           const smoothEaseT = easeT * easeT * (3 - 2 * easeT); // Smoothstep function
            ...
            // FIXED: Target rise should be the calculated spiral rise, not the raw rise
            // This ensures proper connection to the main inner line
            const targetRise = rise;
          ...
          if (parameters.topLength <= 0) {
            ...
            const startZ = innerRadius * Math.sin(spiralEndAngleRad);
                         ...
                         // FIXED: Inner spiral end height should be LOWER than final height
             // The inner spiral ends before the easements, so it needs to account for easement rise
             // This ensures the inner spiral and easement connect properly
             const spiralEndArcDistance = (spiralEndAngle / parameters.totalDegrees) * parameters.totalArcDistance;
             const spiralEndT = spiralEndArcDistance / parameters.totalArcDistance;
             ...
             if (Object.keys(safeManualRiseData).length > 0) {
               ...
               if (lowerPoint === upperPoint) {
                 ...
                 const scaleFactor = safeTotalRise / 7.375;
                 spiralEndRise = insidePitchBlockOffset + (scaledRise * scaleFactor);
               } else {
                 const interpolationFactor = (spiralEndArcDistance - lowerPoint) / (upperPoint - lowerPoint);
                 const baseRise = lowerRise + (upperRise - lowerRise) * interpolationFactor;
